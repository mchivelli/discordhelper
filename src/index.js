...
client.on(Events.ClientReady, () => {
  ...
  cron.schedule(process.env.SYNC_CRON || '0 3 * * *', async () => {
    ...
    try {
      ...
      logger.error('Daily channel sync failed:', err);
    }
  });
  ...
  try {
    ...
    if (!hasState || hasState.count === 0) {
      ...
      setTimeout(() => runDailySync(client, db).catch(err => logger.error('First-run backfill failed:', err)), 10000);
    }
  } catch (err) {
    logger.warn('Could not check sync_state for first-run backfill:', err);
  }
        ...
        classifyToxicity(content).then(classification => {
          if (shouldFlagToxicity(classification.classification, classification.confidence)) {
            db.prepare(`
              ...
              (id, guild_id, channel_id, message_id, user_id, username, content, classification, confidence, details)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).run(
              crypto.randomUUID(),
              message.guild.id,
        ...
        detectQuestion(content).then(detection => {
          if (detection.isQuestion && detection.confidence > 0.7) {
            db.prepare(`
              ...
              (id, guild_id, channel_id, message_id, user_id, username, content, question_type, confidence)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).run(
              crypto.randomUUID(),
              message.guild.id,
