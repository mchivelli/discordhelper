...
module.exports = {
  ...
  async createTask(interaction) {
    try {
      ...
      `).run(
        taskId, title, description, isUnassigned ? TASK_STATUS.UNASSIGNED : TASK_STATUS.IN_PROGRESS, interaction.user.id,
      ...
      // Create compact message in main channel (no buttons here - keep channel clean)
      const statusPrefix = isUnassigned ? PREFIXES.TASK.UNASSIGNED : PREFIXES.TASK.IN_PROGRESS;
      const assigneeText = isUnassigned ? 'Unassigned' : assignees.map(u => u.username).join(', ');
      const compactMessage = `${statusPrefix} **${title}** • ${assigneeText} • \`${taskId}\``;

      const taskMessage = await todoChannel.send(compactMessage);

      // Create thread with status prefix
      // Adding extra length check to prevent title truncation issues with prefix logic
      const prefixLen = statusPrefix.length + 1;
      const titleLimit = 100 - prefixLen;
      const threadName = `${statusPrefix} ${title.substring(0, titleLimit)}`;
      ...
      // Update database with thread_id and message_id
      db.prepare('UPDATE admin_tasks SET thread_id = ?, message_id = ? WHERE task_id = ?')
        .run(thread.id, taskMessage.id, taskId);

      // Post task details inside the thread
      const detailsEmbed = new EmbedBuilder()
        .setTitle(title)
        .setDescription(description)
        .setColor(isUnassigned ? COLORS.GRAY : COLORS.ORANGE)
        .addFields(
          { name: 'Created By', value: `<@${interaction.user.id}>`, inline: true },
          { name: 'Assigned To', value: isUnassigned ? 'Unclaimed' : assignees.map(u => `<@${u.id}>`).join(', '), inline: true }
        )
        .setFooter({ text: `Task ID: ${taskId}` })
        .setTimestamp(timestamp);

      // Create action buttons INSIDE the thread
      ...
      await thread.send({ embeds: [detailsEmbed], components: [actionRow] });

      // Notify assignees in thread
  ...
  async listTasks(interaction) {
    try {
      ...
      for (const task of tasks.slice(0, 10)) {
        ...
        const statusPrefix = {
          'not_started': PREFIXES.TASK.UNASSIGNED,
          'unassigned': PREFIXES.TASK.UNASSIGNED,
          'in_progress': PREFIXES.TASK.IN_PROGRESS,
          'complete': PREFIXES.TASK.COMPLETE
        }[task.status] || '[?]';

        embed.addFields({
          name: `${statusPrefix} ${task.title}`,
        ...
        if (task.thread_id) {
          ...
          if (channel && channel.isThread()) {
            ...
            await interaction.followUp({
              content: `SAFETY ABORT: The stored thread_id points to a regular channel, not a thread. The channel was NOT deleted for safety. Please contact an admin to fix the database.`,
              ephemeral: true
            });
          }
        }
    ...
    try {
      ...
      for (const task of tasks) {
        try {
          ...
          let statusPrefix = PREFIXES.TASK.IN_PROGRESS;
          let embedColor = COLORS.ORANGE;

          if (task.status === 'complete') {
            statusPrefix = PREFIXES.TASK.COMPLETE;
            embedColor = COLORS.GREEN;
          } else if (isUnassigned) {
            statusPrefix = PREFIXES.TASK.UNASSIGNED;
            embedColor = COLORS.GRAY;
          }

          // Create compact message in main channel
          const assigneeText = isUnassigned ? 'Unassigned' : assignees.map(a => `<@${a.user_id}>`).join(', ');
          const compactMessage = `${statusPrefix} **${task.title}** • ${assigneeText} • \`${task.task_id}\``;
          const taskMessage = await todoChannel.send(compactMessage);

          // Create thread with status prefix
          const prefixLen = statusPrefix.length + 1;
          const threadName = `${statusPrefix} ${task.title.substring(0, 100 - prefixLen)}`;
          const thread = await taskMessage.startThread({
            name: threadName,
          ...
          const detailsEmbed = new EmbedBuilder()
            ...
            .setTimestamp(task.created_at);

          // Create action buttons for inside the thread
          const actionRow = new ActionRowBuilder();
          ...
          await thread.send({ embeds: [detailsEmbed], components: [actionRow] });
          await thread.send(`Task Recovered - This task was recovered from the database.`);
      ...
      report += `\n\nIMPORTANT: Update line 7 in \`src/commands/admintasks.js\`:\n\`\`\`javascript\nconst TODO_CHANNEL_ID = '${newChannelId}';\n\`\`\``;
        ...
        if (!thread) {
          try {
            ...
            if (!thread && parent && parent.threads && typeof parent.threads.fetchActive === 'function') {
              const expectedNames = [
                `${PREFIXES.TASK.IN_PROGRESS} ${task.title.substring(0, 93)}`,
                `${PREFIXES.TASK.COMPLETE} ${task.title.substring(0, 93)}`,
                `${PREFIXES.TASK.UNASSIGNED} ${task.title.substring(0, 93)}`,
                `Task: ${task.title}`, // Legacy support
              ...
              if (active && active.threads) {
                thread = active.threads.find(t => expectedNames.some(name => t.name.startsWith(name) || t.name === name)) || thread;
              }
              ...
              if (!thread && typeof parent.threads.fetchArchived === 'function') {
                ...
                if (archivedPublic && archivedPublic.threads) {
                  thread = archivedPublic.threads.find(t => expectedNames.some(name => t.name.startsWith(name) || t.name === name)) || thread;
                }
                if (!thread) {
                  ...
                  if (archivedPrivate && archivedPrivate.threads) {
                    thread = archivedPrivate.threads.find(t => expectedNames.some(name => t.name.startsWith(name) || t.name === name)) || thread;
                  }
                }
              }
    ...
    try {
      ...
      for (let idx = 0; idx < tasks.length; idx++) {
        ...
        if (!thread && task.channel_id && (Date.now() - start) < maxMs) {
          try {
            ...
            if (parent && parent.threads) {
              const expectedNames = [
                `${PREFIXES.TASK.IN_PROGRESS} ${task.title.substring(0, 50)}`, // fuzzy match
                `Task: ${task.title.substring(0, 50)}`
              ];
              const active = await withTimeout(parent.threads.fetchActive());
              if (active && active.threads) {
                thread = active.threads.find(t => expectedNames.some(name => t.name.startsWith(name))) || thread;
              }
              if (!thread && typeof parent.threads.fetchArchived === 'function') {
                const archivedPublic = await withTimeout(parent.threads.fetchArchived({ type: 'public' }));
                if (archivedPublic && archivedPublic.threads) {
                  thread = archivedPublic.threads.find(t => expectedNames.some(name => t.name.startsWith(name))) || thread;
                }
                if (!thread) {
                  const archivedPrivate = await withTimeout(parent.threads.fetchArchived({ type: 'private' }));
                  if (archivedPrivate && archivedPrivate.threads) {
                    thread = archivedPrivate.threads.find(t => expectedNames.some(name => t.name.startsWith(name))) || thread;
                  }
                }
              }
            }
        ...
        if ((Date.now() - start) < maxMs) try {
          const expectedNames = [
            `Task: ${task.title.substring(0, 50)}`,
            `${PREFIXES.TASK.IN_PROGRESS} ${task.title.substring(0, 50)}`
          ];
      ...
      lines.push('\nTips: Ensure the bot has ViewChannel + ReadMessageHistory (+ ManageThreads for private archived) on the original parent channels.');
